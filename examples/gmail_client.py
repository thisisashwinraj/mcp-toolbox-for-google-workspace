...
gmail_mcp_server_params = StdioServerParameters(
    command="uv",
    args=[
        "run",
...
async def gmail_client():
    try:
        async with stdio_client(gmail_mcp_server_params) as (
            gmail_read,
            gmail_write,
        ):
            async with ClientSession(
                ...
                gmail_mcp_tools = await mcp_session.list_tools()

                gmail_tools = types.Tool(
                    function_declarations=[
                        ...
                        for tool in gmail_mcp_tools.tools
                    ]
                )
                ...
                while True:
                    ...
                    response = generate_response(contents, gmail_tools)
...
if __name__ == "__main__":
    asyncio.run(gmail_client())

    """
    EXAMPLE USAGE:
    ---
    >>> Enter your query: Show me the last three unread emails from my inbox.
    ---
    >>> Response:
    Here are your three most recent unread emails:

    1.  **Subject:** Project Update - Q3 Deliverables  
        *   From: `manager@company.com`  
        *   Date: `Aug 27, 2025, 9:42 AM`  

    2.  **Subject:** Invitation: Tech Talk on AI Agents  
        *   From: `events@techconf.org`  
        *   Date: `Aug 27, 2025, 7:15 AM`  

    3.  **Subject:** Your Amazon Order Has Shipped  
        *   From: `orders@amazon.in`  
        *   Date: `Aug 26, 2025, 11:50 PM`  
    """
